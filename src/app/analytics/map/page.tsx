...
import { MapPeriodControl } from "@/components/analytics/map-period-control";
...
function resolvePeriod(raw: string | string[] | undefined): Period {
  return (PERIODS as readonly string[]).includes(raw as string) ? (raw as Period) : "This Month";
}
...
export default async function ReviewMapAnalysisPage({
  ...
  const [businesses, regions, mapMarkers] = await Promise.all([
    getBusinessCounts(),
    getRegionStats(period),
    getBusinessMapMarkers(),
  ]);

  // Derived from the same period-bound `regions` rather than the
  // separate, always-all-time getReviewCounts() — businesses.location
  // is a required column, so every review's business has one, meaning
  // summing regions' counts is already the true total for this period.
  const totalReviews = regions.reduce((sum, r) => sum + r.reviewCount, 0);
  const ratedReviews = regions.filter((r) => r.averageRating !== null);
  const ratedCount = ratedReviews.reduce((sum, r) => sum + r.reviewCount, 0);
  const averageRating = ratedCount
    ? Math.round((ratedReviews.reduce((sum, r) => sum + (r.averageRating ?? 0) * r.reviewCount, 0) / ratedCount) * 10) / 10
    : null;
  ...
  const overviewStats: OverviewStat[] = [
    ...
    { label: "Average Rating", value: averageRating !== null ? String(averageRating) : "—" },
  ];
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <MapPeriodControl period={period} />
          </div>
